...
public class RocketSolarCheck {
    ...
    /**
     * Process solar check for the given player
     * @param player Player who is flying
     */
    private void processSolarCheck(Player player) {
        ...
        if (enhancement != RocketEnhancement.Enhancement.SOLAR)
            return;

        boolean showSolarParticles = false;

        long time = player.getWorld().getTime();
        int flyBase = rp.getBootData().getPower();
