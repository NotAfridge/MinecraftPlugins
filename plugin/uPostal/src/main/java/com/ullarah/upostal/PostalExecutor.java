...
class PostalExecutor implements CommandExecutor {
                    ...
                    if (!getMaintenanceCheck()) {
                        if (args.length >= 1) {
                            if (args[0].matches("[\\w\\d_]{1,16}")) {
                                ...
                                try {
                                    UUID playerID = PlayerProfile.lookup(args[0]).getId();
                            ...
                            } else messageSend(getPlugin(), sender, true, new String[]{ChatColor.RED + "Not a valid player name!"});
                        } else messageSend(getPlugin(), sender, true, new String[]{ChatColor.YELLOW + "Usage: /post <player>"});
    ...
    private void postalCommands(CommandSender sender, String[] args) {
        ...
        String consoleTools = pluginPrefix(getPlugin()) + "clear | maintenance";
